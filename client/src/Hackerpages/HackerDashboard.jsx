...
const FeedDashboard = () => {
 ...
 useEffect(()=>{
  console.log("bsdjbfbfj",logs.map((log)=>{
    return log;
  }))
 },[logs])
  ...
  <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-700 shadow-lg hover:shadow-2xl transition-all duration-300">
    <div className="flex items-center gap-4 flex-wrap">
      ...
      <div>
        ...
        <p className="text-blue-400 text-sm"><span className='text-white'>Welcome Back</span> @{hacker.username || 'anonymous'}</p>
      </div>
    </div>
          ...
          { logs.map((log, idx) => (
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <p className={`px-2 py-1 rounded text-white text-sm font-medium
  ${
    log.reportId.status === "submitted" ? "bg-blue-500" :
    log.reportId.status === "under review" ? "bg-orange-800" :
    log.reportId.status === "triage" ? "bg-yellow-500 text-black" :
    log.reportId.status === "completed" ? "bg-green-500" :
    log.reportId.status === "spam" ? "bg-gray-500 text-black" :
    log.reportId.status === "rejected" ? "bg-red-500" :
