...
const ChatRoom = () => {
  ...
  const sendMessage = () => {
    ...
    if (socket && input.trim()) {
      ...
      editorRef.current?.editor?.commands.clearContent(true) // Clear editor
    }
  }
  ...
  return (
    <div className=" ">
      <div className="ProseMirror space-y-4 px-4 py-2 rounded-lg border-2 border-[#042d5b] overflow-y-auto bg-neutral-800 h-[50vh] top-0 fill-amber-400">
        {messages.map((msg, index) => {
          const isSender = msg.senderInfo._id === currentUserId
          const senderName =
            msg.senderInfo?.name || msg.senderInfo?.username || 'Unknown'
          ...
          return (
            ...
            >
              ...
              <div
                className={`chat-bubble shadow-2xl ${
                  isSender ? 'bg-neutral-700' : 'bg-neutral-700'
                }`}
                dangerouslySetInnerHTML={{ __html: msg.message }}
              />
              <div className="chat-footer opacity-50">
